    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
            if DoesEntityExist(BrancardObject) then
                ...
                TriggerServerEvent('esx-radialmenu:server:RemoveBrancard', PlayerPos, BrancardObject)
                IsAttached = false
                BrancardObject = nil
                IsLayingOnBed = false
            end
    ...
    while true do
        ...
        if BrancardObject ~= nil then
            ...
            elseif Distance <= 2 then
                if not IsLayingOnBed then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, 'Go stand here')
                else
                    if not IsAttached then
...
RegisterNetEvent('esx-radialmenu:client:RemoveBrancardFromArea')
AddEventHandler('esx-radialmenu:client:RemoveBrancardFromArea', function(PlayerPos, BObject)
    local Ped = PlayerPedId()
...
function LayOnBrancard()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('esx-radialmenu:Brancard:BusyCheck', GetPlayerServerId(player), "lay")
        else
            LoadAnim(inBedDicts)
            if Object ~= nil or Object ~= 0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
RegisterNetEvent('esx-radialmenu:Brancard:client:BusyCheck')
AddEventHandler('esx-radialmenu:Brancard:client:BusyCheck', function(OtherId, type)
    local ped = PlayerPedId()
    if type == "lay" then
...
RegisterNetEvent('esx-radialmenu:client:Result')
AddEventHandler('esx-radialmenu:client:Result', function(IsBusy, type)
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)
...
function AttachToBrancard()
    ...
    if BrancardObject ~= nil then
        if ClosestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('esx-radialmenu:Brancard:BusyCheck', GetPlayerServerId(ClosestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(BrancardObject)
                LoadAnim("anim@heists@box_carry@")
                TaskPlayAnim(PlayerPed, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
